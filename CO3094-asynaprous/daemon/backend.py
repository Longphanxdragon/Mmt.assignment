...
mode_async = os.getenv("ASYNAPROUS_MODE", "threading").strip().lower()
if mode_async == "asyncio":
...
async def async_server(ip="0.0.0.0", port=7000, routes={}):
    ...
    async_server = await asyncio.start_server(_handle_client, ip, port)
        ...
        while True:
            if mode_async == "callback":
                ...
                for key, mask in events:
                    ip_sel, port_sel, routes_sel = key.data
                    conn, addr = key.fileobj.accept()
                    client_thread = threading.Thread(
                        target=handle_client_callback,
                        args=(key.fileobj, ip_sel, port_sel, conn, addr, routes_sel),
                    )
